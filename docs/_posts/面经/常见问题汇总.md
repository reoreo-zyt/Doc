...
[基于用户权限的按需加载路由](https://juejin.cn/post/7415653658707329051?searchId=20241202044417E4D663516AFDAEA9BDC6)
[让后端开发赞不绝口的 API 封装技巧：用 Axios 实现高效前端请求管理](https://juejin.cn/post/7407733722881933347)

- 前端基于后端返回的角色信息动态渲染
- 存储一部分静态路由：404 登录页面
  - 定义一个异步路由，asyncRoutes 递归过滤用户有权限的路由
- 路由守卫：beforeEach（前置路由守卫）
  - 用户登录时：判断用户是否有访问当前页面的权限，若无权限则进行重定向。
  - 第一次进入页面或刷新页面时：检查并重新加载用户信息，同时加载动态路由（根据用户权限生成）
  - 用户未登录时：拦截访问请求，若请求页面在白名单内则放行，否则重定向到登录页。
- 自定义权限指令 v-permissions & v-roles
  - 在不具备权限的时候，删除该元素
